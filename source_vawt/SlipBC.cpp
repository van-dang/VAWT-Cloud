			 ...
			 uint node, Array<uint>& nodes)
{
  ...
  if ((n_type == 3 && nsdim == 3) || (n_type == 2 && nsdim == 2)) {
    ...
    if (fabs(fabs(n1) - maxn) < stol) {       // n1 is a largest component
      r1 = nodes[0];
      r2 = nodes[1];
    }
    else if (fabs(fabs(n2) - maxn) < stol) {  // n2 is a largest component
      r1 = nodes[1];
      r2 = nodes[0];
    }
    ...
    */
    ...
    if (n_type == 2 && nsdim == 3) {
      ...
      int non0 = 0;
      if (fabs(n1)<stol) non0++;
      if (fabs(n2)<stol) non0++;
      if (fabs(n3)<stol) non0++;
	...
	nn1 = t11; nn2 = t12; nn3 = t13;
      ...
      if (fabs(fabs(nn1) - maxn) < stol) {       // n1 is a largest component
	r1 = nodes[0];
	r2 = nodes[1];
      }
      else if (fabs(fabs(nn2) - maxn) < stol) {   // n2 is a largest component
	r1 = nodes[1];
	r2 = nodes[0];
      }
      else {                         // n3 is a largest component
	...
	r3 = nodes[0];
      ...
      if (fabs(fabs(tt1) - maxn) < stol) {       // tt1 is a largest component
	r2 = nodes[0]; 
	r3 = (r1 == nodes[1] ?  nodes[2] : nodes[1]) ;
	if (r2 == r1) {
	  real mm = fmax(fabs(tt2), fabs(tt3));
	  if ( fabs(fabs(tt2) - mm) < stol) {
	    r2 = nodes[1]; 
	    r3 = nodes[2];
	  }
	  else {
      ...
      else if (fabs(fabs(tt2) - maxn) < stol) {  // tt2 is a largest component
	r2 = nodes[1];
	r3 = (r1 == nodes[0] ? nodes[2] : nodes[0]) ;
	if (r2 == r1) {
	  real mm = fmax(fabs(tt1), fabs(tt3));
	  if (fabs(fabs(tt1) - mm) < stol) {
	    r2 = nodes[0]; 
	    r3 = nodes[2];
	  }
	  else {
	...
	if (r3 == a1_indices[i])
      ...
      uint row_d1 = 0;
      uint row_d2 = 0;
      if (fabs( fabs(row_d11) - maxr) < stol  || fabs( fabs(row_d22) - maxr) < stol) {
	row_d1 = r2;
	row_d2 = r3;
      }
      else {
